...
namespace MyPhotoSlideshow
{
    public partial class PhotoNavigator : UserControl
    {
        ...
        public PhotoNavigator()
        {
            ...
            rssURL = HtmlPage.Document.GetElementById("rssURL").GetAttribute("value");// +"&co=Finland&ci=Helsingfors";
        ...
        {
            UriBuilder ub = new UriBuilder(in_rss + "&format=json&thnsize=" + ThumbnailSize.Width);
            ...
            wc.OpenReadCompleted += (a, b) =>
                {
                    if (b.Result != null)
                    {
                        ...
                        using (StreamReader sr = new StreamReader(s))
                        {
                            ...
                            foreach (var obj in wlbobj.entries)
                            {
        ...
        private void AddImage(dynamic obj)
        {
            if (obj.link!=null)
            {
                PhotoInfo pi = new PhotoInfo(obj, true);
                if (pi.link.Length > 0)
                {
                    Image img = new Image { Name = pi.title, Width = ThumbnailSize.Width, Height = ThumbnailSize.Height, Margin = new Thickness(1), Stretch = System.Windows.Media.Stretch.Uniform };
                    img.Opacity = .01;
        ...
        private void changeDate(DateTime in_dt)
        {
            if (!currentDateTime.Equals(in_dt))
            {
                ...
                if (!per_page.Equals("NaN"))
                    GetImages(rssURL.Replace("#per_page#", per_page) + "&ym=" + in_dt.ToString("yyyy-MM"));
        ...
        private void LayoutRoot_SizeChanged(object sender, SizeChangedEventArgs e)
        {
            ...
            string per_page = Math.Max(100, getPerPage()).ToString();
            if(!per_page.Equals("NaN"))
                GetImages(rssURL.Replace("#per_page#", per_page) + "&ym=" + dpDate.Text);
        }
